...
Esta actividad incluye una evaluación automática similar a la de la actividad anterior, adaptada a `git worktree` (alta de worktrees, trabajo paralelo, creación con `-b` y limpieza con `remove`/`prune`):
...
- `required_files` (por defecto: `README.md,cp1-historial-worktree.txt,cp2-worktree-list.txt,cp3-trabajo-paralelo.txt,cp4-feature-b-worktree.txt,cp5-remove-prune.txt,reflexion-6-13.md`)
