...
Extracto Reflexion 6.13 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Esta actividad (6.13) se califica con rubrica: Worktree tecnico (max 4), Evidencias (max 3), Reflexion (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece worktree_checks.cp2_worktree_feature_a.ok en true, reconoce explicitamente el uso de `git worktree add` y `git worktree list` para feature-a.
- Si en JSON aparece worktree_checks.cp4_feature_b_add_b.ok en true, reconoce explicitamente el uso de `git worktree add -b` para crear feature-b.
- Si en JSON aparece worktree_checks.cp5_remove_prune.ok en true, reconoce explicitamente la gestion de limpieza con `git worktree remove` y `git worktree prune`.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-13.md'))
