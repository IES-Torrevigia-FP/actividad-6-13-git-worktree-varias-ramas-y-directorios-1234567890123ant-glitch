...
def analyze_worktree_activity(root: Path) -> dict:
    cp1_path = root / 'cp1-historial-worktree.txt'
    cp2_path = root / 'cp2-worktree-list.txt'
    cp3_path = root / 'cp3-trabajo-paralelo.txt'
    cp4_path = root / 'cp4-feature-b-worktree.txt'
    cp5_path = root / 'cp5-remove-prune.txt'
    ...
    readme = safe_read_text(readme_path).lower()

    cp1_historial_ok = contains_any(cp1, ['git log --oneline --all']) and contains_any(cp1, ['git branch', 'feature-a'])

    cp2_worktree_list_ok = contains_any(cp2, ['git worktree list', 'wt-feature-a', 'feature-a'])
    cp2_estructura_ok = contains_any(cp2, ['principal', 'directorio', 'worktree'])

    cp3_paralelo_ok = contains_any(cp3, ['git worktree list']) and contains_any(cp3, ['main', 'feature-a'])

    cp4_add_b_ok = contains_any(cp4, ['git worktree add -b', 'feature-b', 'wt-feature-b'])
    cp4_list_ok = contains_any(cp4, ['git worktree list'])
    cp4_explain_ok = contains_any(cp4, ['git branch', 'add -b', 'diferencia'])

    cp5_remove_ok = contains_any(cp5, ['git worktree remove', 'wt-feature-a'])
    cp5_before_after_ok = contains_any(cp5, ['antes', 'despues', 'git worktree list'])
    cp5_prune_explain_ok = contains_any(cp5, ['prune', 'remove'])

    readme_min_content_ok = contains_any(readme, ['proyecto worktree'])

    return {
        'cp1_historial_ramas': {
            'ok': cp1_historial_ok,
            'source': 'cp1-historial-worktree.txt' if cp1_path.exists() else 'missing',
        },
        'cp2_worktree_feature_a': {
            'ok': cp2_worktree_list_ok and cp2_estructura_ok,
            'source': 'cp2-worktree-list.txt' if cp2_path.exists() else 'missing',
        },
        'cp3_trabajo_paralelo': {
            'ok': cp3_paralelo_ok,
            'source': 'cp3-trabajo-paralelo.txt' if cp3_path.exists() else 'missing',
        },
        'cp4_feature_b_add_b': {
            'ok': cp4_add_b_ok and cp4_list_ok and cp4_explain_ok,
            'source': 'cp4-feature-b-worktree.txt' if cp4_path.exists() else 'missing',
        },
        'cp5_remove_prune': {
            'ok': cp5_remove_ok and cp5_before_after_ok and cp5_prune_explain_ok,
            'source': 'cp5-remove-prune.txt' if cp5_path.exists() else 'missing',
        },
        'readme_final': {
            'exists': readme_path.exists(),
            'minimum_content_ok': readme_min_content_ok,
        },
    }
...
def main():
    ...
    ap.add_argument('--required', default='README.md,cp1-historial-worktree.txt,cp2-worktree-list.txt,cp3-trabajo-paralelo.txt,cp4-feature-b-worktree.txt,cp5-remove-prune.txt,reflexion-6-13.md')
    ...
    worktree_info = analyze_worktree_activity(root)
    ...
    reflex_path = root / 'reflexion-6-13.md'
    ...
    evidencia_candidates = [
        'cp1', 'cp2', 'cp3', 'cp4', 'cp5',
        'worktree', 'feature-a', 'feature-b', 'remove', 'prune', 'reflexion',
    ]
    ...
    checks_worktree = [
        worktree_info['cp1_historial_ramas']['ok'],
        worktree_info['cp2_worktree_feature_a']['ok'],
        worktree_info['cp3_trabajo_paralelo']['ok'],
        worktree_info['cp4_feature_b_add_b']['ok'],
        worktree_info['cp5_remove_prune']['ok'],
        worktree_info['readme_final']['exists'] and worktree_info['readme_final']['minimum_content_ok'],
    ]
    tech_hits = sum(1 for x in checks_worktree if x)
    ...
    evidence_hits = sum(1 for x in evidence_checks if x)
    if evidence_hits >= 4:
        s_evidencias = 3
    ...
    total = s_worktree + s_evidencias + s_reflex
    ...
    resumen = {
        ...
        'scores': {
            'worktree_tecnico (max_4)': s_worktree,
            'evidencias (max_3)': s_evidencias,
    ...
    with (outdir / 'metricas.csv').open('w', newline='', encoding='utf-8') as f:
        ...
        w.writerow(['worktree_tecnico (max 4)', s_worktree])
        w.writerow(['evidencias (max 3)', s_evidencias])
    ...
    md.append('|---|---:|')
    md.append(f"| Uso tecnico de git worktree (add/list/remove/prune) | {s_worktree}/4 |")
    md.append(f"| Evidencias de trabajo en paralelo y estructura | {s_evidencias}/3 |")
    md.append(f"| Reflexion 6.13 | {s_reflex}/3 |")
    ...
    md.append('## Checks de worktree')
    md.append(f"- Historial + ramas base (cp1): {'si' if worktree_info['cp1_historial_ramas']['ok'] else 'no'}  ")
    md.append(f"- Worktree de feature-a (cp2): {'si' if worktree_info['cp2_worktree_feature_a']['ok'] else 'no'}  ")
    md.append(f"- Trabajo paralelo en worktrees (cp3): {'si' if worktree_info['cp3_trabajo_paralelo']['ok'] else 'no'}  ")
    md.append(f"- Alta de feature-b con add -b (cp4): {'si' if worktree_info['cp4_feature_b_add_b']['ok'] else 'no'}  ")
    md.append(f"- Remove/prune correctamente explicados (cp5): {'si' if worktree_info['cp5_remove_prune']['ok'] else 'no'}  ")
    md.append(f"- README final presente y minimo valido: {'si' if worktree_info['readme_final']['exists'] and worktree_info['readme_final']['minimum_content_ok'] else 'no'}  ")
    md.append('')
